...
interface UseEnhancedPermissionsReturn extends EnhancedPermissionsState {
...
export function useEnhancedPermissions(): UseEnhancedPermissionsReturn {
  const { user } = useAuth();
    ...
    (check: PermissionCheck): PermissionResult => {
      ...
      if (
        effective.roles.some(
          (role) => role.slug === "super_user" && role.isSystemRole
        )
      ) {
        const result = {
          granted: true,
...
export function useRoleGuard(
  ...
  const { hasRole, hasAnyRole, hasAllRoles } = useEnhancedPermissions();
