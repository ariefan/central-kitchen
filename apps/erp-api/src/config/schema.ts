...
// Tenancy & users
// ---------------------------------------------------------------------------
export const tenants = erp.table(
  "tenants",
  {
...
export const locations = erp.table(
  "locations",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
// ---------------------------------------------------------------------------
export const userLocations = erp.table(
  "user_locations",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: uuid("user_id")
...
// Document sequences
// ---------------------------------------------------------------------------
export const docSequences = erp.table(
  "doc_sequences",
  {
  ...
  (t) => ({
    uqSeq: unique("uq_docseq_tenant_type_period_loc").on(
      t.tenantId,
      t.docType,
      t.period,
      t.locationId
...
// Partners
// ---------------------------------------------------------------------------
export const suppliers = erp.table(
  "suppliers",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
...
export const customers = erp.table(
  "customers",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
  ...
  (t) => ({
    ...
    uqTenantAuth: unique("uq_customer_tenant_auth").on(
      t.tenantId,
      t.authUserId
    ),
  })
...
export const uomConversions = erp.table(
  "uom_conversions",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
  ...
  (t) => ({
    uqPair: unique("uq_conv_tenant_pair").on(
      t.tenantId,
      t.fromUomId,
      t.toUomId
    ),
...
// ---------------------------------------------------------------------------
export const products = erp.table(
  "products",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
...
export const productVariants = erp.table(
  "product_variants",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    ...
    priceDifferential: varchar("price_differential", { length: 32 })
      .notNull()
      .default("0"), // Money amount (can be negative)
    barcode: varchar("barcode", { length: 255 }),
  ...
  (t) => ({
    uqProdVariant: unique("uq_variant_prod_name").on(
      t.productId,
      t.variantName
    ),
    idxDisplayOrder: index("idx_variant_display_order").on(
      t.productId,
      t.displayOrder
    ),
  })
...
export const productPacks = erp.table(
  "product_packs",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    productId: uuid("product_id")
    ...
    toBaseFactor: numeric("to_base_factor", {
      precision: 16,
      scale: 6,
    }).notNull(),
    isDefault: boolean("is_default").notNull().default(false),
...
export const supplierProducts = erp.table(
  "supplier_products",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (t) => ({
    uqSupplierProduct: unique("uq_supplier_product").on(
      t.supplierId,
      t.productId
    ),
...
export const modifierGroups = erp.table(
  "modifier_groups",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
...
export const modifiers = erp.table(
  "modifiers",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const productModifierGroups = erp.table(
...
export const lots = erp.table(
  ...
  (t) => ({
    idxLot: index("idx_lot_prod_loc_exp").on(
      t.productId,
      t.locationId,
      t.expiryDate
    ),
    uqScoped: unique("uq_lot_tenant_prod_loc_no").on(
      t.tenantId,
      t.productId,
      t.locationId,
      t.lotNo
...
export const stockLedger = erp.table(
  "stock_ledger",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    tenantId: uuid("tenant_id")
    ...
    qtyDeltaBase: numeric("qty_delta_base", {
      precision: 18,
      scale: 6,
    }).notNull(),
  ...
  (t) => ({
    idxProdLocTs: index("idx_ledger_prod_loc_ts").on(
      t.productId,
      t.locationId,
      t.txnTs
    ),
...
export const costLayers = erp.table(
  "cost_layers",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
    ...
    locationId: uuid("location_id")
      ...
      .references(() => locations.id, { onDelete: "restrict" }),
    lotId: uuid("lot_id").references(() => lots.id, { onDelete: "set null" }),
    qtyRemainingBase: numeric("qty_remaining_base", {
      precision: 18,
      scale: 6,
    }).notNull(),
  ...
  (t) => ({
    idxKey: index("idx_cost_layer_key").on(
      t.tenantId,
      t.productId,
      t.locationId,
      t.lotId
...
export const costLayerConsumptions = erp.table(
...
// Purchasing (PO → GR)
// ---------------------------------------------------------------------------
export const purchaseOrders = erp.table(
  "purchase_orders",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
  ...
  (t) => ({
    ...
    ckTotals: check(
      "ck_po_totals_nonneg",
      sql`${t.subtotal} >= 0 AND ${t.taxAmount} >= 0 AND ${t.totalAmount} >= 0`
    ),
  })
);

export const purchaseOrderItems = erp.table(
  ...
  (t) => ({
    ...
    ckQtyPos: check("ck_poi_qty_pos", sql`${t.quantity} > 0`),
  })
);

export const goodsReceipts = erp.table(
  "goods_receipts",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
...
export const goodsReceiptItems = erp.table(
  "goods_receipt_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (t) => ({
    ...
    ckQtyPos: check("ck_gri_qty_pos", sql`${t.quantityReceived} > 0`),
  })
);
...
// ---------------------------------------------------------------------------
export const requisitions = erp.table(
  "requisitions",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
    ...
    issueStatus: varchar("issue_status", { length: 24 })
      .notNull()
      .default("pending"), // pending|partial|fully_issued
    notes: text("notes"),
    metadata: jsonb("metadata"),
    createdAt: timestamp("created_at").notNull().defaultNow(),
    updatedAt: timestamp("updated_at").notNull().defaultNow(),
  ...
  (t) => ({
    ...
    idxTransfer: index("idx_req_transfer").on(t.transferId),
    idxIssueStatus: index("idx_req_issue_status").on(t.issueStatus),
  })
);

export const requisitionItems = erp.table(
  "requisition_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    ...
    qtyIssued: numeric("qty_issued", { precision: 16, scale: 6 })
      .notNull()
      .default("0"),
    notes: text("notes"),
    createdAt: timestamp("created_at").notNull().defaultNow(),
  ...
  (t) => ({
    ...
    ckQtyReqPos: check("ck_req_qtyreq_pos", sql`${t.qtyRequested} > 0`),
  })
);

export const transfers = erp.table(
  "transfers",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
...
export const transferItems = erp.table(
  "transfer_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (t) => ({
    ...
    ckQtyPos: check("ck_xfer_qty_pos", sql`${t.quantity} > 0`),
  })
);
...
// Production
// ---------------------------------------------------------------------------
export const recipes = erp.table(
  "recipes",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
    ...
    finishedProductId: uuid("finished_product_id")
      .notNull()
      .references(() => products.id, { onDelete: "restrict" }),
    yieldQtyBase: numeric("yield_qty_base", {
      precision: 16,
      scale: 6,
    }).notNull(),
...
export const recipeItems = erp.table(
  "recipe_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (t) => ({
    ...
    ckQtyPos: check("ck_recipe_qty_pos", sql`${t.qtyBase} > 0`),
  })
);

export const productionOrders = erp.table(
  "production_orders",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
    ...
    locationId: uuid("location_id")
      .notNull()
      .references(() => locations.id, { onDelete: "restrict" }),
    plannedQtyBase: numeric("planned_qty_base", {
      precision: 16,
      scale: 6,
    }).notNull(),
    producedQtyBase: numeric("produced_qty_base", { precision: 16, scale: 6 })
      .notNull()
      .default("0"),
...
export const orders = erp.table(
  "orders",
  {
    ...
    kitchenStatus: varchar("kitchen_status", { length: 16 })
      .notNull()
      .default("open"),
    ...
    discountAmount: numeric("discount_amount", { precision: 16, scale: 2 })
      .notNull()
      .default("0"),
    serviceChargeAmount: numeric("svc_amount", { precision: 16, scale: 2 })
      .notNull()
      .default("0"),
    tipsAmount: numeric("tips_amount", { precision: 16, scale: 2 })
      .notNull()
      .default("0"),
    voucherAmount: numeric("voucher_amount", { precision: 16, scale: 2 })
      .notNull()
      .default("0"),
    totalAmount: numeric("total_amount", { precision: 16, scale: 2 })
      .notNull()
...
export const orderItems = erp.table(
  "order_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    ...
    discountAmount: numeric("discount_amount", { precision: 16, scale: 2 })
      ...
      .default("0"),
    lineTotal: numeric("line_total", { precision: 16, scale: 2 }).notNull(),
    prepStatus: varchar("prep_status", { length: 16 })
      .notNull()
      .default("queued"),
  ...
  (t) => ({
    ...
    ckQtyPos: check("ck_order_qty_pos", sql`${t.quantity} > 0`),
  })
);

export const orderItemModifiers = erp.table(
...
export const payments = erp.table(
  "payments",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    orderId: uuid("order_id")
      .notNull()
      .references(() => orders.id, { onDelete: "cascade" }),
    tender: varchar("tender", { length: 24 }).notNull(),
    amount: numeric("amount", { precision: 16, scale: 2 }).notNull(),
    reference: varchar("reference", { length: 128 }),
    change: numeric("change", { precision: 16, scale: 2 })
      .notNull()
      .default("0"),
...
export const posShifts = erp.table(
  "pos_shifts",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
    ...
    floatAmount: numeric("float_amount", { precision: 16, scale: 2 })
      .notNull()
      .default("0"),
    expectedCash: numeric("expected_cash", { precision: 16, scale: 2 })
      .notNull()
      .default("0"),
    actualCash: numeric("actual_cash", { precision: 16, scale: 2 })
      .notNull()
      .default("0"),
    variance: numeric("variance", { precision: 16, scale: 2 })
      .notNull()
      .default("0"),
  },
...
export const drawerMovements = erp.table(
  "drawer_movements",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const carts = erp.table("carts", {
  ...
  deviceId: varchar("device_id", { length: 64 }),
  channel: varchar("channel", { length: 16 }).default("online"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
// ---------------------------------------------------------------------------
export const returnOrders = erp.table(
  "return_orders",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
    ...
    returnType: varchar("return_type", { length: 24 }).notNull(),
  ...
  (t) => ({
    uqReturnNo: unique("uq_return_tenant_no").on(t.tenantId, t.returnNumber),
    idxStatus: index("idx_return_status").on(t.status),
  })
);

export const returnOrderItems = erp.table(
  "return_order_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (t) => ({
    idxReturn: index("idx_return_items").on(t.returnOrderId),
    ckQtyPos: check("ck_return_qty_pos", sql`${t.quantity} > 0`),
  })
);

export const stockAdjustments = erp.table(
  "stock_adjustments",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
...
export const stockAdjustmentItems = erp.table(
...
export const stockCounts = erp.table(
  "stock_counts",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
...
export const stockCountLines = erp.table(
  "stock_count_lines",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    ...
    productId: uuid("product_id")
      ...
      .references(() => products.id, { onDelete: "restrict" }),
    lotId: uuid("lot_id").references(() => lots.id, { onDelete: "set null" }),
    systemQtyBase: numeric("system_qty_base", {
      precision: 16,
      scale: 6,
    }).notNull(),
    countedQtyBase: numeric("counted_qty_base", {
      precision: 16,
      scale: 6,
    }).notNull(),
    varianceQtyBase: numeric("variance_qty_base", {
      precision: 16,
      scale: 6,
    }).notNull(),
    createdAt: timestamp("created_at").notNull().defaultNow(),
...
export const temperatureLogs = erp.table(
  "temperature_logs",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
...
export const vouchers = erp.table(
  "vouchers",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
...
// ---------------------------------------------------------------------------
export const taxCategories = erp.table(
  "tax_categories",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    tenantId: uuid("tenant_id")
