...
export const requisitionService = {
  ...
  async create(rawBody: unknown, context: RequestContext) {
    ...
    if (body.fromLocationId === body.toLocationId) {
      throw new RequisitionServiceError('Cannot requisition from and to the same location');
    }
    ...
    const requisition = await db.transaction(async (tx) => {
      const reqNumber = generateDocNumber('REQ', { tenantId: context.tenantId });
    ...
    return requisitionRepository.findDetailedById(requisition, context.tenantId);
  },

  async update(id: string, rawBody: unknown, context: RequestContext) {
    const body = requisitionUpdateSchema.parse(rawBody ?? {});
    const requisition = await requisitionRepository.findById(id, context.tenantId);
    if (!requisition) {
      return null;
    }
    assertStatus(requisition.status, ['draft'], 'Requisition not found or cannot be edited');

    if (body.fromLocationId && body.fromLocationId !== requisition.fromLocationId) {
    ...
    if (body.requiredDate !== undefined) {
      payload.requiredDate = body.requiredDate ? new Date(body.requiredDate) : null;
    }
    if (body.notes !== undefined) {
      payload.notes = body.notes ?? null;
    }

    await requisitionRepository.updateRequisition(id, context.tenantId, payload);
    return requisitionRepository.findDetailedById(id, context.tenantId);
  },
  ...
  async approve(id: string, context: RequestContext) {
    ...
    assertStatus(requisition.status, ['draft'], 'Requisition not found or already processed');
