...
export function uomConversionRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      let whereConditions = [eq(uomConversions.tenantId, currentUser.tenantId)];
      ...
      const allConversions = await db.select({
        ...
        fromUom: uoms,
        toUom: { id: uoms.id, name: uoms.name, code: uoms.code, symbol: uoms.symbol, uomType: uoms.uomType },
      })
        .from(uomConversions)
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      ...
      const conversionData = await db.select({
        ...
        .where(
          and(
            eq(uomConversions.id, request.params.id),
            eq(uomConversions.tenantId, currentUser.tenantId)
          )
        )
        .limit(1);
  ...
  fastify.post(
    ...
    async (request: FastifyRequest<{ Body: z.infer<typeof uomConversionCreateSchema> }>, reply: FastifyReply) => {
      ...
      // Verify both UOMs exist and belong to tenant
      const fromUom = await db.select()
        .from(uoms)
        .where(and(
          eq(uoms.id, request.body.fromUomId),
          eq(uoms.tenantId, currentUser.tenantId)
        ))
        .limit(1);

      const toUom = await db.select()
        .from(uoms)
        .where(and(
          eq(uoms.id, request.body.toUomId),
          eq(uoms.tenantId, currentUser.tenantId)
        ))
        .limit(1);

      if (!fromUom.length || !toUom.length) {
        return createBadRequestError('One or both UOMs not found', reply);
      }
      ...
      if (existingConversion.length) {
        return createBadRequestError('Conversion between these UOMs already exists', reply);
      }
